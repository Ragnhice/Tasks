...
def pytest_generate_tests(metafunc):
    ...
    def load_from_module(module):
        return importlib.import_module(
            "data.%s" % module).testdata  # получение тестовых данных
    module = load_from_module("core")
    module_classes = {}
    for key, data in inspect.getmembers(module, inspect.isclass):
        module_classes[key] = data

    for class_item in module_classes.keys():
        class_functions = inspect.getmembers(module.class_item, inspect.isfunction)
    ...
    return data
    #тестовая ф. принимает словарь и выводит информацию только об 1 методе, я бы вывела список методов и классов





    #dir_path = os.path.dirname(os.path.abspath(metafunc.module.__file__))    # Определяем директорию текущего файла
    #file_path = os.path.join(dir_path, metafunc.function.__name__ + '.yaml')    #?? Определяем путь к файлу с классами
